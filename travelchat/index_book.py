...
def process_news_data_in_vector(article):
    article["url"] = "https://www.cicerone.co.uk/annapurna-2"
    print(f"Indexing article: {article.get('title')} in {article.get('url')}")
    ...
    chunks = [
        {
            "id": create_composite_key(f"{article.get("url")}-{article.get("title")}", section_index, chunk_index),
            "text": (
                f'#{article.get("title")}\n\n'
